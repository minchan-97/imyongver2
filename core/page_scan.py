...
마인드맵·개념도·흐름도·표 같은 도식이면, 낱말만 나열하지 말고 관계가 보이게 옮긴다.
  - 중심 주제를 첫 줄에 쓰고, 가지를 들여쓰기 목록으로 (중심 → 대분류 → 세부)
  - 화살표·선으로 이어진 것은 'A → B', 묶인 것은 같은 들여쓰기 단계로
  - 마지막에 한 줄로 "이 도식은 …을 정리한 것"이라고 요약을 덧붙인다(내용에 근거해서만)
